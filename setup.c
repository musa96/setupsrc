...
char inkey, keys[14] = {0xc8, 0xd0, 0xcb, 0xcd, 0x33, 0x34, 0x39, 0x2a,
    			0xc9, 0xd1, 0xc7, 0x1e, 0x2c, 0x1d};
...
#define OLDCFGFILE "SN.OLD"
#define RUNEXE "SN.EXE"
...
void warpto(int episode)
{
	...
	printf(";  Skill level: %s    ;\n", skill[gameskill]);
        ...
        switch (inkey)
        {
			...
			tempbuf[0] = "sn.exe";
			tempbuf[1] = "-e";
			sprintf(string[2], "%d", episode);
			tempbuf[2] = string[2];
			tempbuf[3] = "-m";
			tempbuf[4] = "1";
			tempbuf[5] = "-s";
			sprintf(string[6], "%d", gameskill);
			tempbuf[6] = string[6];
                        spawnv(P_WAIT, RUNEXE, tempbuf);
			exit(0);
                        break;
                case '2':
			_asm
		...
		case 45:
		gameskill--;
		if (gameskill < 0) gameskill = 0;
		clrscr();	
                warpto(episode);
		break;
		case 61:
		gameskill++;
		if (gameskill > 4) gameskill = 4;
		clrscr();	
                warpto(episode);
		break;
		case 46:
